...
router.post('/mua', auth, (req, res) => {
	let { _id, type, balance, ten_goi, money, limit_nhom, limit_vip, limit_fanpage, limit_account } = req.body;
			...
			if (res_exist_goi <= 0) {
				listUser.findOneAndUpdate(
					...
					listUser.findOneAndUpdate(
						...
						res.json({"status": "200", "msg": "Mua <strong>" + ten_goi + "</strong> thành công"});
					});
				});
			} else {
				listUser.findOneAndUpdate(
					...
					res.json({"status": "200", "msg": "Gia hạn <strong>" + ten_goi + "</strong> thành công"});
				});
			}
		});
	} else {
		res.json({"status": "400", "msg": "Số dư không đủ để mua <strong>" + ten_goi + "</strong>"});
	}
});
