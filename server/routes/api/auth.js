		...
		User.findOne({name})
			.then(user => {
				...
				bcrypt.compare(password, user.password)
					.then(isMatch => {
						if(!isMatch) return res.status(400).json({ msg: 'Tài khoản hoặc mật khẩu không đúng' });
	...
	try{
		...
		.then((useradmin) => {
			if (useradmin.role == 1) {
				...
				.then(item => item.remove().then(() => res.json({status: 200,msg:'Xóa tài khoản thành công'})))
				.catch((err) => {
					console.log(err);
					res.status(400).json({status: 400,msg:'Xóa tài khoản không thành công'})
				});
			}
		})
				...
				bcrypt.genSalt(10, (err, salt) => {
					bcrypt.hash(password, salt, (err, hash) => {
						...
						User.findOneAndUpdate(query, body, {upsert: true}, function(err, doc) {
						    ...
						    res.json({status: 200,msg:'Cập nhật mật khẩu thành công'})
						});
					});
				});
			} else {
...
router.post('/user_save_info',auth, (req, res) => {
	try{
		...
		if (req.user.id == _id) {
			if (old_pass == '') {
				return res.send({error:400, msg: 'Mật khẩu cũ không được để trống!'});
			}
			var date = new Date();
			User.findById(req.user.id)
			.then((user) => {
				// Create salt & hash
				bcrypt.compare(old_pass, user.password)
					.then(isMatch => {
						if(!isMatch) return res.send({error:400, msg: 'Mật khẩu cũ không đúng!'});
						
						if (new_pass != '') {
							if (new_pass != re_pass) {
								return res.send({error:400, msg: 'Mật khẩu nhập lại không khớp!'});
							}
						}
						...
						bcrypt.genSalt(10, (err, salt) => {
							bcrypt.hash(new_pass, salt, (err, hash) => {
								...
								User.findOneAndUpdate(query, body, {upsert: true}, function(err, doc) {
									if (err) return res.send(500, {error: err});
									return res.json({status: 200,msg:'Cập nhật mật khẩu thành công'})
								});
							});
						});
					})
			...
			return res.send({error:400, msg: 'Đây không phải là tài khoản của bạn!'});
		}
	} catch(e) {
		console.log(e);
	}
