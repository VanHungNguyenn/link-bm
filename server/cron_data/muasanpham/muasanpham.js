	...
	User.find({_id: body.id_user})
		...
		.then((items) => {
			...
			if (items.length > 0) {
				...
				tongtienmua = Math.abs(parseInt(items[0].tongtienmua) + (price_buy * sl));
				...
				Product.find({id_loaisp: body.id_category,sell:0,status:1})
						.sort({date: 1})
						.limit(body.sl)
						.then( async product => {
							if (product.length >= parseInt(body.sl)) {
								...
								for (var i = 0; i < product.length; i++) {
									checkpoint = await checkUID(product[i].data.split('|')[0]);
									if (checkpoint == 1) {
										var query = {'_id': product[i]._id};
										Product.findOneAndUpdate(query, {status: 2}, {upsert: true}, function(err, doc) {});
								...
								Product.updateMany({ _id: { $in: ids } }, data, {upsert: true}, function(err, doc) {
									User.updateMany({ _id:  body.id_user}, {balance: balance,tongtienmua:tongtienmua}, {upsert: true}, function(err, doc) {
										const newLogs = new Logs({
											id_user: body.id_user,
											user_name: body.user_name,
											soluongmua: parseInt(body.sl),
											id_loaisanpham: body.id_category,
											name_category: body.name_category,
											description_category: body.description_category,
											price_buy: tientong,
											id_sanpham: ids
										});
...
function updateActionBuy(id_giaodich, msg, status){
	...
	MuaSanPham.findOneAndUpdate(query, data, {upsert: true}, function(err, doc) {
	    getBank();
	});
}
