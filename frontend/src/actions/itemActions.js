...
export const  getItemsAdmin= (listtype = '') => (dispatch, getState) => {
	const token = getState().auth.token;
	const dataPass = {
	    headers: {
	    	"Content-type": "application/json",
	...
	if (token !== null) {
		...
		axios.get('/api/fbmark/notifilogs', dataPass).then(res =>{
			if (res.data.status === 200) {
				var mess = 'Bạn đã nạp thành công số tiền '+res.data.data[0].tien_nap+' VNĐ';
				dispatch(returnErrors(mess, res.data.status, 'NAP_TIEN_SUCCESS'));
				dispatch(loadUser());
			}
		})
	}
...
export const layGiaoDich = () => (dispatch, getState) => {
	...
	if (token !== null) {
		...
		axios.get('/api/fbmark/thongtingiaodich', dataPass).then(res =>{
			if (res.data.status === 200) {
				var mess = res.data.msg;
				dispatch(returnErrors(mess, res.data.status, 'GIAO_DICH_SUCCESS'));
				dispatch(loadUser());
			} else {
				var mess = res.data.msg;
				// dispatch(layGiaoDich());
				dispatch(returnErrors(mess, res.data.status, 'GIAO_DICH_FAIL'));
				dispatch(loadUser());
			}
		}).catch((err)=>{
			dispatch(layGiaoDich());
...
export const getProductByCategory = (listtype = '') => (dispatch, getState) => {
	const token = getState().auth.token;
	const dataPass = {
	    params: {
	      	idUser: getState().auth.user._id,
...
export const cLinkBM = (linkbm, luong, cookie, token_bm) => (dispatch, getState) => {
	const token = getState().auth.token;
	const config = {
		headers: {
			'Content-Type': 'application/json',
	...
	axios.post('/api/fbmark/checklinkbm', body, config)
		.then(res => {
			if (res.data.status === 200) {
...
export const cIDBM = (id_bm, luong, cookie, token_bm) => (dispatch, getState) => {
	var data = {
		businessID: id_bm,
		linkbm : ''
	}
...
export const checklinkbm1 = (data, cookie, token) => (dispatch, getState) => {
	...
	axios.get('https://graph.facebook.com/v4.0/'+data.businessID, dataPass).then(res =>{
...
export const checklinkbm2 = (data, cookie) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/checklinkbmstep', body, config)
		.then(res => {
...
export const muaTaikhoan = (id_category, sl, totalpriceuser, name_category, description_category, price_buy, tienmua) => (dispatch, getState) => {
	...
	var encrypted = CryptoJS.AES.encrypt(JSON.stringify(body), CryptoJS.enc.Base64.parse('khongthehackduoc'),options).ciphertext.toString(CryptoJS.enc.Base64);
	axios.post('/api/fbmark/buyproduct', {params:encrypted}, config)
		.then(res => {
			if (res.data.status === 200) {
				dispatch(layGiaoDich());
				dispatch(returnErrors(res.data.msg, res.data.status, 'MUA_TK_SUCCESS'));
			} else {
				dispatch(returnErrors(res.data.msg, res.data.status, 'MUA_TK_FAIL'));
			}
			dispatch(getItems());
			dispatch(loadUser());
		})
...
export const saveCategoryProduct = (data) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/addcategoryproduct', data, config)
		.then(res => {
			if (res.data.status == 200) {
				dispatch(returnErrors(res.data.msg,200, 'ADD_CATEGORY_SUCCESS'));
				dispatch(getItems());
			} else {
				dispatch(returnErrors(res.data.msg,400, 'ADD_CATEGORY_TK_FAIL'));
			}
		})
		.catch(err => {
			dispatch(returnErrors('Có lỗi xảy ra khi lưu loại sản phẩm',400, 'ADD_CATEGORY_TK_FAIL'));
		});
};

export const saveProduct = (data, listtype = '') => (dispatch, getState) => {
	const token = getState().auth.token;
	const config = {
		headers: {
			'Content-Type': 'application/json',
	...
	axios.post('/api/fbmark/addproduct', data, config)
		.then(res => {
			if (res.data.status == 200) {
				dispatch(returnErrors(res.data.msg,200, 'ADD_PRODUCT_SUCCESS'));
				dispatch(getItems());
				dispatch(getItemsAdmin(listtype));
			} else {
				dispatch(returnErrors(res.data.msg,400, 'ADD_PRODUCT_TK_FAIL'));
			}
		})
		.catch(err => {
			dispatch(returnErrors('Có lỗi xảy ra khi lưu sản phẩm',400, 'ADD_PRODUCT_TK_FAIL'));
		});
};

export const editStatusProduct = (data, listtype = '') => (dispatch, getState) => {
	const token = getState().auth.token;
	const config = {
		headers: {
			'Content-Type': 'application/json',
	...
	axios.post('/api/fbmark/editstatusproduct', data, config)
		.then(res => {
			if (res.data.status == 200) {
				dispatch(returnErrors(res.data.msg,200, 'EDIT_STATUS_PRODUCT_SUCCESS'));
				dispatch(getItems());
				dispatch(getItemsAdmin(listtype));
			} else {
				dispatch(returnErrors(res.data.msg,400, 'EDIT_STATUS_PRODUCT_TK_FAIL'));
			}
		})
		.catch(err => {
			dispatch(returnErrors('Có lỗi xảy ra khi lưu sản phẩm',400, 'EDIT_STATUS_PRODUCT_TK_FAIL'));
		});
};
...
export const saveThongBao = (des) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/addnotifi', data, config)
		.then(res => {
			if (res.data.status == 200) {
		...
		.catch(err => {
			dispatch(returnErrors('Có lỗi xảy ra khi thêm thông báo',400, 'ADD_NOTIFI_FAIL'));
		});
};
export const saveConfig = (data) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/addnotifi', data, config)
		...
		.catch(err => {
			dispatch(returnErrors('Có lỗi xảy ra khi lưu thông báo',400, 'SAVE_NOTIFI_FAIL'));
		});
};
export const deleteCategoryProduct = (ids) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/deletecategoryproduct', ids, config)
		.then(res => {
			if (res.data.status == 200) {
				dispatch(returnErrors(res.data.msg,200, 'DELETE_CATEGORY_SUCCESS'));
				dispatch(getItems());
			}
		})
		.catch(err => {
			dispatch(returnErrors('Có lỗi xảy ra khi xóa thông báo',400, 'DELETE_CATEGORY_TK_FAIL'));
		});
};
export const xoaThongbao = (ids) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/deletenotifi', ids, config)
		.then(res => {
			if (res.data.status == 200) {
				dispatch(returnErrors(res.data.msg,200, 'DELETE_NOTIFI_SUCCESS'));
				dispatch(laythongbaoadmin());
			}
		})
		.catch(err => {
			dispatch(returnErrors('Có lỗi xảy ra khi xóa thông báo',400, 'DELETE_NOTIFI_FAIL'));
		});
};
export const deleteProductMulti = (ids) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/deleteproduct', ids, config)
		.then(res => {
			if (res.data.status == 200) {
				dispatch(returnErrors(res.data.msg,200, 'DELETE_PRODUCT_SUCCESS'));
				dispatch(getItems());
				dispatch(getItemsAdmin('chuaban'));
			}
		})
		.catch(err => {
			dispatch(returnErrors('Có lỗi xảy ra khi xóa sản phẩm',400, 'DELETE_PRODUCT_TK_FAIL'));
		});
};
export const downloadProduct = (ids, title, sl) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/downloadproduct', body, config)
		.then(res => {
			if (res.data.length > 0) {
				var content = '';
		...
		.catch(err => {
			dispatch(returnErrors('Có lỗi xảy ra khi tải xuống',400, 'DOWNLOAD_TK_FAIL'));
		});
};
export const veiwProduct = (ids) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/viewproduct', body, config)
		.then(res => {
			dispatch({
				type: 'VIEW_PRODUCT',
				payload: res.data
		...
		.catch(err => {
			dispatch(returnErrors('Có lỗi xảy ra khi xem chi tiết',400, 'VIEW_PRODUCT_FAIL'));
		});
};
...
export const veiwProductAdmin = (ids) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/viewproductadmin', body, config)
		.then(res => {
			dispatch({
				type: 'VIEW_PRODUCT',
				payload: res.data
...
export const getTotalMoney = (data) => (dispatch, getState) => {
	...
	axios.post('/api/fbmark/gettotalmoney', data, config)
		.then(res => {
			if (res.data.status == 200) {
				dispatch({
					type: "TOTAL_MONEY",
...
export const getProductLoi = () => (dispatch, getState) => {
	...
	axios.get('/api/fbmark/getproductloi', config)
		.then(res => {
			if (res.data.status == 200) {
				dispatch({
