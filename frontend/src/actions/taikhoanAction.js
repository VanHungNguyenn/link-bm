...
import { TAIKHOAN_LOADED, PUSH_EDIT_TAIKHOAN } from './types'

import toastr from 'reactjs-toastr'
import { returnErrors } from './errorActions'
export const getListDataTaiKhoan = () => (dispatch, getState) => {
	const config = tokenConfig(getState)
	axios.get('/api/taikhoan/', config).then((res) => {
		dispatch({
...
export const editTaiKhoanfunc = (body) => (dispatch, getState) => {
	axios
		.post('/api/taikhoan/edit', body, tokenConfig(getState))
		.then((res) => {
			if (res.data.status === 200) {
				toastr.success(res.data.msg, 'Thành công', {
