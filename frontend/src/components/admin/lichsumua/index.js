...
import {laylichsumuaadmin, laylichsumua, veiwProductAdmin, downloadProduct} from '../../../actions/itemActions';
	...
	render() {
	    ...
	    const columns = [
			...
			},
			{
				...
				formatter:(cell, row, rowIndex) => <Fragment>{row.description_category.slice(0, 30)}</Fragment>
			},
			{
				text: 'Số lượng',
			...
			},
			{
				text: 'Tổng giá',
				dataField: 'price_buy',
				formatter:(cell, row, rowIndex) => <Fragment>{this.formatMoney(row.price_buy)} VNĐ</Fragment>
			},
			{
				text: 'Ngày mua',
				dataField: 'ngaymua',
			...
			},
			{
				text: 'Xem chi tiết',
				dataField: '',
				formatter:(cell, row, rowIndex) => <Fragment><button className="btn btn-sm btn-primary "  data={row.index} onClick={()=>{this.xemChiTiet(row.id_sanpham)}}>
	                            <i className="si si-eye"></i> Xem
	                          </button></Fragment>
			},
			{
				text: 'Hành động',
				formatter:(cell, row, rowIndex) => <Fragment><button className="btn btn-sm btn-primary " data={row.index} onClick={()=>{this.downloadData(row.id_sanpham, row.name_category, row.soluongmua)}}>
...
export default connect(mapStateToProps, {laylichsumuaadmin, laylichsumua, veiwProductAdmin, downloadProduct })(AdminLichSuMua);