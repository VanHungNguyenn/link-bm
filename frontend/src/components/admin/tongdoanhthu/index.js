...
import {laylichsunapadmin, getTotalMoney, laytongdoanhthu} from '../../../actions/itemActions';
...
class AdminLichSuNapTien extends Component {
	...
	render() {
		const { admindespit, totalmoney, admintongdoanhthu } = this.props.item;
		var new_labels = [];
		var new_datasets = [];
		var new_labelsv2 = [];
		var new_datasetsv2 = [];
		...
		if (admintongdoanhthu.length > 0) {
			groups = admintongdoanhthu[0].lichsunaptien.reduce((groups, game) => {
			...
			groupsv2 = admintongdoanhthu[0].lichsumua.reduce((groupsv2, game) => {
				...
				<div className="content">
                    <div className="block block-rounded block-bordered">
						<div className="block-header block-header-default border-bottom">
							<h3 className="block-title">Tổng doanh thu hàng tháng</h3>
						</div>
...
export default connect(mapStateToProps, {laylichsunapadmin, getTotalMoney, laytongdoanhthu})(AdminLichSuNapTien);