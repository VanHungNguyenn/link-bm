	...
	render() {
	   ...
	   const columns = [
			...
			},
			{
				text: 'Tên tài khoản',
				dataField: 'data',
				formatter:(cell, row, rowIndex) => <Fragment>{row.data.user_name}</Fragment>
			},
			{
				text: 'Tiền trong tài khoản',
				dataField: 'data',
				formatter:(cell, row, rowIndex) => <Fragment>{row.data.price+row.data.price_buy*row.data.sl}</Fragment>
			},
			{
				text: 'Số lượng mua',
				dataField: 'data',
				formatter:(cell, row, rowIndex) => <Fragment>{row.data.sl}</Fragment>
			},
			{
				text: 'Tiền mua sản phẩm',
				dataField: 'data',
				formatter:(cell, row, rowIndex) => <Fragment>{row.data.price_buy*row.data.sl}</Fragment>
			},
			{
				text: 'Lỗi',
				dataField: 'data',
				formatter:(cell, row, rowIndex) => <Fragment>{row.msg.msg}</Fragment>
			},
			{
				text: 'Ngày lỗi',
				dataField: 'data',
		...
		return (
			<Fragment>
				...
				<div className="block-header block-header-default border-bottom">
                    <h3 className="block-title">Quản lý sản phẩm lỗi</h3>
                </div>
