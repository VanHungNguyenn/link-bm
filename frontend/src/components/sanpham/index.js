			...
			if (cell === 1) {
				return (
					<span className='badge badge-success'>
						<i className='fas fa-check mr-1'></i>Acc OK
					</span>
				)
			} else if (cell === 2) {
				return (
					<span className='badge badge-danger'>
						<i className='fas fa-times mr-1'></i>Acc Die
					</span>
				)
			} else {
				return ''
		...
		const columns = [
			...
			},
			{
				...
				formatter: (cell, row, rowIndex) => (
					<Fragment>{row.data.slice(0, 30)}</Fragment>
				),
			},
			{
				text: 'Loại sản phẩm',
			...
			},
			{
				...
				formatter: (cell, row, rowIndex) => (
					<Fragment>
						{row.date_sell === null
							? ''
							: this.convertDate(row.date_sell)}
					</Fragment>
				),
			},
			{
				...
				formatter: (cell, row, rowIndex) => (
					<Fragment>
						<button
							...
							data={row.index}
							onClick={() => {
								this.openStatusProduct(row._id, row.index)
							}}
						>
					...
					>
						...
						<div className='block-content'>
											...
											<div className='row mb-4'>
												<div className='col-md-12'>
													...
													<button
														...
														onClick={() => {
															this.clickGetProductByCategory()
														}}
													>
														Tất cả tài khoản
													</button>
							...
							<Modal
								className='bt-customWidth-dtable'
								toggle={(e) => {
									this.openModalScanGroupsStatus(
										!this.state
											.isModalBrowseGroupsStatusOpen,
										e
									)
								}}
								isOpen={
									this.state.isModalBrowseGroupsStatusOpen
								}
							>
								<div className='modal-header'>
									<p className='mb-0 text-dark'>
