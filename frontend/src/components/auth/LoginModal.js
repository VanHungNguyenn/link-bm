...
class LoginModal extends Component {
	...
	render() {
		...
		var thongbao = ('');
		if (notifi.thongbao !== '') {
			thongbao = (<div className="alert alert-primary" role="alert">
				<p className="mb-0">Thông báo từ Admin: {notifi.thongbao}!</p>
			</div>);
		}
		...
		var tabuser = (
			<Fragment>
				<li className="nav-item">
					<NavLink to="/" activeClassName="active" className="nav-link text-body-color py-4" id="tabKey" onClick={() => {this.dropdownMenu('')}}>
						<i className="nav-main-link-icon si si-home text-gray"></i>
						<span className="d-none d-md-inline ml-1">Trang chủ</span>
					</NavLink>
				</li>
				<li className="nav-item dropdown">
	                <Link activeClassName="active" id="contact" className="nav-link text-body-color py-4" onClick={() => { this.dropdownMenu('show')}} onBlur={(e) => { this.dropdownMenuBlur(e)}}>
						<i className="nav-main-link-icon si si-users text-gray"></i>
						<span className="d-none d-md-inline ml-1">Liên hệ</span>
					</Link>
					<div className={"dropdown-menu " + (this.state.menuDropdown)} aria-labelledby="dropdown-default-primary" x-placement="bottom-start">
                        <a rel="noopener noreferrer" className="dropdown-item" href={notifi.link_group_fb}>Vào nhóm facebook</a>
                        <a rel="noopener noreferrer" className="dropdown-item" href={notifi.link_group_zalo}>Vào nhóm zalo</a>
                        <a rel="noopener noreferrer" className="dropdown-item" href={notifi.link_group_tele}>Vào nhóm tele</a>
                        <a rel="noopener noreferrer" className="dropdown-item" href={notifi.link_backup}>Link backup via</a>
                    </div>
				</li>
			</Fragment>
		)
						...
						<div className="block-content block-content-full  bg-white" style={{"padding":"20px !important"}}>
							...
							<form className="js-validation-signin user" onSubmit={this.onSubmit} style={{"padding":"0px 10% 0px 10%"}}>
								...
								<div className="form-group text-center">
									<button onClick={this.clearAlert} type="submit" className="btn btn-hero-primary">
		...
		return (
			<Router>
				<Fragment>
					<div id="page-container" className="page-header-dark main-content-boxed side-trans-enabled">
						<header id="page-header">
							<div className="content-header">
								<div>
									<Link className="link-fx font-size-lg text-dual" to="/">
										<span className="font-w300 text-dual text-uppercase">{domainSite}</span>
									</Link>
								</div>
							</div>
						...
						<footer id="page-footer" className="bg-white border-top">
							<div className="content py-0">
								<div className="row font-size-sm">
									<div className="col-sm-6 order-sm-1 text-center text-sm-left">
										<a className="font-w600 text-uppercase" href={"https://" + domainSite} target="_blank">{domainSite}</a> © <span data-toggle="year-copy" className="js-year-copy-enabled">2020</span>
									</div>
								</div>
							</div>
						</footer>
